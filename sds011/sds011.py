...
while n<1:
    ...
    if previousbyte == HEADER_BYTE and byte == COMMANDER_BYTE:
        ...
        time.sleep (1)
        subprocess.run(["mosquitto_pub", "-h", "192.168.1.125", "-t", "air-quality/pm10", "-m", str(pm_10), "-u", "mqtt", "-P", "maxmil"])
        time.sleep (1)
        subprocess.run(["mosquitto_pub", "-h", "192.168.1.125", "-t", "air-quality/pm25", "-m", str(pm_25), "-u", "mqtt", "-P", "maxmil"])
        n=n+1
